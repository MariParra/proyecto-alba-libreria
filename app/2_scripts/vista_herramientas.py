...
def sync_google_sheets():
    try:
        ...
        secreto = st.secrets["gcp_service_account"]
        
        if "credentials" in secreto:
            creds_str = secreto["credentials"]
            # Reparación en caso de que Streamlit lo haya leído como diccionario
        ...
        # Filtro final: Si private_key sigue siendo un diccionario por error humano, mostramos aviso claro
        if isinstance(creds_dict.get("private_key"), dict):
            st.error("🚨 Error de formato: Asegúrate de que tu JSON en Streamlit Secrets esté pegado exactamente entre TRES comillas simples (''')")
            return
        ...
        st.error(f"Error al conectar con Google Sheets. Revisa los Secretos. Detalle técnico: {e}")
        return
