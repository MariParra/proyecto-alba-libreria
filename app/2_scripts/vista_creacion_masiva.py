...
def procesar_nuevos_libros(df):
    ...
    for indice, fila in df_clean.iterrows():
        ...
        try:
            ...
            # Diagnóstico temporal en pantalla
            st.write(f"🔍 **Resultado Fila {indice + 2} ('{titulo_excel}'):**", res.data)

            if res.data and len(res.data) > 0:
                exitos += 1
                catalogo_actual.append((titulo_excel.lower(), autor_excel.lower()))
            else:
                errores.append(f"Fila {indice + 2} ('{titulo_excel}'): La BD aceptó la petición pero no guardó la fila (revisar políticas RLS en Supabase).")
