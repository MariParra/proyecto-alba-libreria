...
def generar_comprobante(
    ...
    """
    Genera un comprobante premium. Es robusto y funciona tanto para carritos nuevos (con desglose)
    como para carritos reconstruidos de ventas antiguas (sin desglose).
    """
    try:
        # --- PASO 1: RECONSTRUCCIÓN FINANCIERA INTELIGENTE ---
        suma_precios_catalogo = 0.0
        suma_precios_cobrados = 0.0
        carrito_procesado = []

        # Determinar si el carrito tiene desglose de descuentos o es uno simple (de historial)
        # Si al menos un item tiene 'precio_original', asumimos que es un carrito moderno.
        is_modern_cart = all('precio_original' in item for item in carrito)

        if is_modern_cart:
            # --- Lógica para carritos nuevos con desglose completo ---
            for item in carrito:
                item_copy = item.copy()
                qty = int(item_copy.get('cantidad', 1))
                
                # Usar los datos de alta fidelidad que ya vienen en el carrito
                suma_precios_catalogo += float(item_copy.get('precio_original', 0.0)) * qty
                suma_precios_cobrados += float(item_copy.get('subtotal', 0.0))
                carrito_procesado.append(item_copy)
        else:
            # --- Lógica de fallback para carritos antiguos (reconstruidos) ---
            # Usa los totales pasados como argumentos, ya que no hay desglose
            suma_precios_cobrados = float(subtotal)
            # Intenta estimar el precio de catálogo. Si no es posible, será igual al cobrado.
            suma_precios_catalogo = float(subtotal)
            carrito_procesado = carrito # Usa el carrito simple tal como viene
        ...
        try:
            ...
            pass # Si falla, se creará un fondo blanco

        if img is None:
            img = Image.new('RGB', (800, 1200), color='#FAF8FC')

        # --- PASO 3: DIBUJO DEL COMPROBANTE ---
        # (El resto del código de dibujo no necesita cambios)
        ...
        # Retorna una imagen de respaldo en caso de error para no romper la UI
        img_fallback = Image.new('RGB', (800, 1200), color='#FAF8FC')
        draw_fallback = ImageDraw.Draw(img_fallback)
        font_fallback = obtener_fuente_comprobante("Lato", 20, bold=True)
        draw_fallback.text((100, 500), "Error al generar el comprobante.", fill='#C62828', font=font_fallback)
        draw_fallback.text((100, 530), f"Detalle: {str(e_global)[:80]}...", fill='#333333', font=font_fallback)
        buf = io.BytesIO()
