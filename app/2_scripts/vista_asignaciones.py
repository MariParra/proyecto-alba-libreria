...
def cambiar_estado_mes(ano, mes, cerrar=True):
    ...
    try:
        if cerrar:
            datos = {"ano": int(ano), "mes": int(mes), "fecha_cierre": datetime.now().isoformat()}
...
def comenzar_mes(ano, mes):
    ...
    for _, cliente in df_suscritos.iterrows():
        try:
            ...
            # Buscar el valor base de su suscripción
            val_sub = 0.0
            ...
            datos = {
                ...
                "valor_envio": 0.0, "monto_total": val_sub,
...
def asignar_libro_a_suscripcion(asignacion_id, cliente_id, libro_id, titulo, autor, ano, mes, stock_actual, libros_extras=[]):
    conn = get_db_connection()
    try:
        ...
        datos_update = {"libro_suscripcion_id": id_libro_py}
        if nombres_extras:
            res_asig = conn.table("asignaciones").select("extras").eq("asignacion_id", id_asig_py).execute()
            extras_actual = res_asig.data[0].get('extras', '') or ''
            nuevo_extra = extras_actual + (" | " if extras_actual else "") + "EXTRAS: " + ", ".join(nombres_extras)
            datos_update["extras"] = nuevo_extra
...
def actualizar_asignaciones_batch(df_editado, df_mes_completo):
    ...
    for a_id, row in filas_cambiadas.iterrows():
        try:
            # Buscar cliente y valor de su suscripción
            ...
            m_total = val_sub + v_envio # Recalcula el total automáticamente
...
def mostrar_asignaciones():
    ...
    if opcion_menu == "📋 Gestión (Tabla Editable)":
        ...
        if df_mes.empty: st.warning("No hay registros para este mes. Ve a 'Comenzar Mes'.")
            ...
            # Agregamos las nuevas columnas financieras
            columnas_mostrar = ['asignacion_id', 'nombre_cliente', 'titulo_libro', 'estado_envio', 'pagado', 'envio_pagado', 'valor_envio', 'monto_total', 'extras', 'comentario']
            ...
            configuracion_columnas = {
                ...
                "valor_envio": st.column_config.NumberColumn("Valor Envío ($)", min_value=0.0, format="$%.0f"),
                "monto_total": st.column_config.NumberColumn("Monto Total ($)", format="$%.0f")
            }
                
            df_editado = st.data_editor(
                df_mostrar, 
                # El monto total se calcula solo, por lo que bloqueamos su edición manual
            ...
            if not df_mostrar.equals(df_editado) and not mes_esta_cerrado:
                if st.button("💾 Guardar Cambios", type="primary"):
                    with st.spinner("Calculando totales y actualizando..."):
            ...
            if df_mes.empty: st.info("No hay suscripciones creadas para este mes.")
                ...
                if not df_pendientes.empty:
                    ...
                    with st.container(border=True):
                        ...
                        # NUEVO: Selector de Libros Extras
                        st.caption("Si eliges libros extra, se descontarán del stock y se añadirán a la columna Extras.")
                        libros_extras_sel = st.multiselect("Libros Extras Adicionales (Opcional):", [t for t in df_libros['titulo'].tolist() if t != ""])
                        
                        if st.button("Asignar estos libros", use_container_width=True):
                            if asig_manual_sel and libro_manual_sel:
                                ...
                                ex, err = asignar_libro_a_suscripcion(id_asig, id_cliente, l_data['libro_id'], l_data['titulo'], l_data.get('autor', ''), ano_sel, mes_num, l_data['stock'], libros_extras_sel)
                                if ex: st.success("¡Libro(s) asignado(s) con éxito!"), st.rerun()
        ...
        if mes_esta_cerrado: st.warning("El mes está cerrado. No puedes generar nuevas filas.")
            ...
            st.info(f"Se crearán filas en blanco para todos los clientes 'SUSCRITO' para {mes_sel} de {ano_sel}. El Monto Total se calculará según la tabla suscripciones.")
            ...
            if not df_mes.empty:
                ...
                if asig_eliminar:
                    if st.button("🟥 ELIMINAR DEFINITIVAMENTE", type="primary"):
                        ...
                        ex, err = eliminar_asignacion(id_asig, row_data.get('libro_suscripcion_id'), row_data['cliente_id'], ano_sel, mes_num, row_data.get('extras', ''))
